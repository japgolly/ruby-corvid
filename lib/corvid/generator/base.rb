    ...
    class Base < Thor
      ...
      def add_features(*feature_ids)
        ...
        feature_ids.flatten.each do |feature|
          unless installed.include?(feature)
            installed<< feature
            p,f = split_feature_id feature
            @@with_resource_features<< f if p == @@with_resource_plugin
          end
        end
      ...
      # Are all provided features installed (or in the process of being installed)?
      #
      # For use in templates and in conjunction with {#regenerate_template_with_feature}.
      #
      ...
      # @raise If resources haven't been made available by {#with_resources} or {#override_installed_features}.
      def all_features_installed?(*features)
        features.flatten.compact.all?{|f| feature_installed? f }
      end

      # Are any provided features installed (or in the process of being installed)?
      #
      # For use in templates and in conjunction with {#regenerate_template_with_feature}.
      #
      # @param [Array<Symbol|String>] feature_names Feature names (not feature ids).
      # @return [Boolean] If any feature names are installed (or in the process of being installed).
      # @raise If resources haven't been made available by {#with_resources} or {#override_installed_features}.
      def any_features_installed?(*features)
        features.flatten.compact.any?{|f| feature_installed? f }
      end
